          ...
          {history.length === 0 ? (
            ...
            history.map((q: string, i: number) => (
              <ul className='flex flex-col px-1 py-0.5 h-full cursor-pointer' key={i}>
                <li onClick={() => setQuestion(q)} className={`text-[13px] truncate w-full p-1 rounded-[5px] ${disablebtn ? "text-zinc-900 disabled" : "text-zinc-200 hover:bg-zinc-700 hover:text-white enabled"}`}>{q}</li>
              </ul>
            ))
          )}
        </section>
