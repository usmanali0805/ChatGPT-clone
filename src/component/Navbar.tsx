...
interface NavbarProps{
  setTemporarychat:(q:boolean)=>void;
  temporarychat:boolean;
}

const Navbar:React.FC<NavbarProps> = ({setTemporarychat , temporarychat}) => {
  const { setMblbtn} = useHistory ()

  const chat_handle = () => setTemporarychat(!temporarychat)
  ...
  return (
    <div className="text-white bg-[#212121] p-3 px-5 absolute z-30 w-full  top-0 left-0 items-center flex justify-between ">
      ...
      <span className='sm:hidden '><ChartNoAxesGantt onClick={()=> setMblbtn(true)} className='p-1 cursor-pointer' /></span>
